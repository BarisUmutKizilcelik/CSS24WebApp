...
export const Predictor = class Predictor {
  ...
  addDatapoint = (sensorName, value, time = null) => {
    if (typeof value !== "number")
      throw new TypeError("Datapoint is not a number");
    if (!this.sensors.includes(sensorName))
      throw new TypeError("Sensor is not valid");
  ...
  _updateStore() {
    if (this.windowModeMs) {
      if (
        this.lastPruneTime + STORE_MAX_DATAPOINT_FACTOR * this.windowSize >
        this.lastAddTime
      ) {
        return;
      }
      ...
      for (const sensorName of this.sensors) {
        this.store[sensorName] = Predictor._sliceByTime(
          this.store[sensorName],
          this.lastAddTime - this.windowSize,
        );
      }
    } else {
      for (const sensorName of this.sensors) {
        if (
          this.store[sensorName].length >
          this.windowSize * STORE_MAX_DATAPOINT_FACTOR * 2
        ) {
          this.store[sensorName] = this.store[sensorName].slice(
            -STORE_MAX_DATAPOINT_FACTOR * this.windowSize,
          );
        }
      }
    }
  ...
  predict = async () => {
    ...
    const [featNames, feats] = await Predictor._extract(
      window,
      this.sensors.length,
      this.scaler,
    );
    ...
    return {
      prediction:
        this.labels[
          pred.reduce((iMax, x, i, arr) => (x > arr[iMax] ? i : iMax), 0)
        ],
      result: pred,
    };
  };
  ...
  static async _extract(frame, sensorsLength, scaler) {
    ...
    feats.sort(([[aI, aFeat]], [[bI, bFeat]]) => {
      ...
      return (
        Predictor.featuresTSfresh.indexOf(aFeat) -
        Predictor.featuresTSfresh.indexOf(bFeat)
      );
    });
    if (scaler) {
...
Predictor.felFeaturesTSfresh = cache(() =>
  arrToVector(Predictor.featuresTSfresh, "string"),
);
